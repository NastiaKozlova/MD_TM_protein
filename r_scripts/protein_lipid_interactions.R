...
for (j in 1:length(v_parta)) {
    ...
    df_energy<-df_energy%>%group_by(resno, segid)%>%mutate(min_Elec=min(Elec))%>%
        mutate(min_VdW=min(VdW))
    df_energy<-df_energy%>%select(resno, segid,x,y,z,       
                                  min_Elec,min_VdW)
    df_energy<-unique(df_energy)
...
#        geom_point(aes(x=resno,y=min_Elec,color="Elec"))+
#        geom_point(aes(x=resno,y=min_VdW,color="VdW"))+
#        facet_grid(segid~.)+
#        theme_bw()
    
#    ggplot(data=df_energy)+
#        geom_point(aes(x=VdW,y=Elec))+
#        theme_bw()
#    ggplot(data=df_energy)+
#        geom_point(aes(x=resno,y=Elec,color="Elec"))+
#        geom_point(aes(x=resno,y=VdW,color="VdW"))+
#        facet_grid(segid~.)+
#        theme_bw()
#    ggplot(data=df_energy)+
#        geom_point(aes(x=z,y=Elec,color="Elec"))+
#        geom_point(aes(x=z,y=VdW,color="VdW"))+
#        theme_bw()
#    #    geom_point(aes(x=Elec,y=VdW))
#    ggplot(data=df_energy)+
#        geom_density(aes(x=Elec))
#    ggplot(data=df_energy)+
#        geom_density(aes(x=VdW))
#    ggplot(data=df_energy)+
#        geom_density(aes(x=Nonbond))
#    df_energy<-df_energy%>%filter(Nonbond<0)
#    ggplot(data=df_energy)+
#        geom_density(aes(x=Nonbond))#
