...
library(dplyr)
setwd(part_start)
part_first<-paste0(part_start,"MD_analysis/docking/docking_first/")
part_name<-paste0(part_start,"MD_analysis/docking/docking_2/")
df_cascade_docking<-read.csv(paste0(part_start,"start/cascade_docking.csv"),stringsAsFactors = F)
df_cascade_docking<-df_cascade_docking%>%mutate(group=paste0(center,"_",ligand_first))
v_separate<-unique(df_cascade_docking$group)
f<-1
p<-1
