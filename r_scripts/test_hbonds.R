...
for (p in 1:length(v_part)) {
  ...
  for (main in main_part) {
    if (!dir.exists(paste0(part,'/din/hbonds/',main,'_seqid/'))){dir.create(paste0(part,'/din/hbonds/',main,'_seqid/'))}
    if (!dir.exists(paste0("hbonds/",main,"_mod"))){dir.create(paste0("hbonds/",main,"_mod"))}
    number_frame<-length(list.files(paste0("pdb_second/hbond_",main,"")))-1 
    if (number_frame>0) { 
      pdb<-read.pdb(paste0("pdb_second/",main,"/frame_0.pdb"))
      v_segid<-unique(pdb$atom$segid)
      
      
      for(y in 1:length(v_segid)){
        print("kek")  
        for (q in 0:number_frame) {
          file_name<-paste0("hbonds/",main,"/seqid_",v_segid[y],"_frame_",q,".txt")
          if(file.exists(file_name)){
            ...
            df_pdb<-df_pdb%>%filter(segid==v_segid[1])
            df_bonds<-left_join(df_bonds,df_pdb,c("number"="resno","amino"="resid"))
            write.csv(df_bonds,paste0("hbonds/",main,"_seqid/seqid_",v_segid[y],"_frame_",q,".txt"),row.names = F)
          }
        }
      }
    }
    y<-1 
    for (q in 0:number_frame) {
      if(file.exists(paste0("hbonds/",main,"_seqid/seqid_",v_segid[1],"_frame_",q,".txt"))){
    ...
    for (q in 1:number_frame) {
      if(file.exists(paste0("hbonds/",main,"_mod/frame_",q,".txt"))){
