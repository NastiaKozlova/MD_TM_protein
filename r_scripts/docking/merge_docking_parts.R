...
for (i in 1:length(v_complex)){
  ...
  for (j in 1:nrow(df_RMSD)) {
    if (!is.na(df_RMSD$name.x[j])) {
      ...
      if (nrow(df_structure_RMSD_complex_test)>5) {
...
for (i in 1:length(v_fin_structure)) {
  ...
  for (j in 1:length(v_structure)) {
    ...
    write.pdb(pdb_1,paste0("structure_merged/",v_fin_structure[i],"/",v_structure[j],".pdb"))
...
df_structure_RMSD<-df_structure_RMSD%>%mutate(size_of_group=n())
#df_structure_RMSD<-df_structure_RMSD%>%select(name.x,receptor,ligand,RMSD.x,name.y,number.x,receptor_ligand,
#                                              grop_number.x,models.x,RMSD.y,models.y,number.y,grop_number.y,
#                                              group,ligand_center,mode,affinity,rmsd,rmsd_from_BM,name_files,
#                                              files,new_number,number)
