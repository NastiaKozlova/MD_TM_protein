...
v_RMSD<-list.files(paste0("MD_analysis/docking/df_RMSD/"))
df_RMSD<-read.csv(paste0("MD_analysis/docking/df_RMSD/",v_RMSD[1]),stringsAsFactors = F)
if(length(v_parta)>1){
  for (j in 2:length(v_parta)) {
    df_RMSD_add<-read.csv(paste0("MD_analysis/docking/df_RMSD/",v_RMSD[j]),stringsAsFactors = F)
