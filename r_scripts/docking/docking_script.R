...
for (j in 1:nrow(df_all)) {
    ...
    df_pdb<-df_pdb%>%mutate(separate=paste0(resno,chain))
    
    df_doking<-df_active_center%>%filter(type==df_all$type[j])
    df_doking<-df_doking%>%mutate(separate=paste0(resno,chain))
    df_pdb_a<-df_pdb[df_pdb$separate%in%df_doking$separate,]
