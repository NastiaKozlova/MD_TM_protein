part<-"/home/nastia/mem/ACHE/"
part_start<-"/home/nastia/mem/ACHE/"
protein<-c("ACHE","BCHE")

library(dplyr)
library(bio3d)
library(ggplot2)
#system(command = paste0("Rscript --vanilla  ",part_start,"r_scripts/download_structures.R ",part_start),ignore.stdout=T,wait = T)
#system(command = paste0("Rscript --vanilla  ",part_start,"r_scripts/chouse_structure.R ",part_start),ignore.stdout=T,wait = T)
...
#system(command = paste0("Rscript --vanilla  ",part_start,"r_scripts/prepare_tcl_din_25.R ",part_start),ignore.stdout=T,wait = T)
#prepare_for_docking
for (name in protein){
v_ligand<-list.files(paste0(part,name,"/start/ligand_start/"))
a<-c()
...
df_active_center<-read.csv(paste0(part,name,"/start/active_center.csv"),stringsAsFactors = F)
...
write.csv(df_ligand_center,paste0(part,name,"/start/ligand_center.csv"),row.names = F)
}
#docking
for (name in protein){

  part_start<-paste0(part,name,"/docking/docking_first/")
  if(!dir.exists(paste0(part,name,"/docking"))){dir.create(paste0(part,name,"/docking"))}
  if(!dir.exists(part_start)){dir.create(part_start)}
  system(command = paste0("cp -r ",part,name,"/start/ligand_start/ ",part_start),ignore.stdout=T,wait = T)
  system(command = paste0("cp ",part,name,"/start/active_center.csv ",part_start),ignore.stdout=T,wait = T)
  system(command = paste0("cp -r ",part,name,"/start/receptor_start/ ",part_start),ignore.stdout=T,wait = T)
...
#  system(command = paste0("/home/nastia/MGLTools-1.5.7/bin/pythonsh /home/nastia/MGLTools-1.5.7/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_receptor4.py -r ",
#                          part_start,"receptor_start/start.pdb -o ",part_start,"receptor/start.pdbqt ",
#                          "-A None"))
...
for (name in protein){
  ...
  ggsave(p,filename = paste0("din/tost.png"), width = 100, height = 20, units = c("cm"), dpi = 200 ) 
  
  
  p<-ggplot(data=df_logu_min)+
    geom_point(aes(x=number,y=affinity,colour=colour))+facet_grid(receptor~ligand_center)+theme_bw()+guides(colour="none")
  ggsave(p,filename = paste0("din/tost_min.png"), width = 100, height = 20, units = c("cm"), dpi = 200 ) 
  
  p<-ggplot(data=df_logu_max)+
    geom_point(aes(x=number,y=affinity,colour=colour))+facet_grid(receptor~ligand_center)+theme_bw()+guides(colour="none")
  ggsave(p,filename = paste0("din/tost_max.png"), width = 100, height = 20, units = c("cm"), dpi = 200 ) 
}
