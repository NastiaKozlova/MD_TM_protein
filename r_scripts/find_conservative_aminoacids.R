...
srart<-list.files(path = paste0(part_start,"start/sequence/"))
start<-NULL
...
rm(srart)
seqname<-start[1]
if(!dir.exists(paste0( "MD_analysis/conservative")))    { dir.create(paste0( "MD_analysis/conservative"))}
for(seqname in start){
  ...
  write.csv(df_ali,paste0("MD_analysis/conservative/",seqname,".csv"),row.names = F)
}
